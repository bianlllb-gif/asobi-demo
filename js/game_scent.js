...
var PAIR=[
  {t:'酸涩。闷了一整天。',        act:'黑丝套上去。裹住龟头。上下动。',     pool:['黑丝']},
  {t:'温热。潮湿的汗息。',        act:'鼻子贴脚底。手握着鸡巴。一起动。',      pool:['赤足','抬脚','拖鞋']},
  {t:'甜。底下是皮肤的咸。',      act:'抬手臂。把鼻子埋进腋下。手别停。',    pool:['腋下','抬手','举臂','汗腋']},
  {t:'皮革混着丝袜。密闭了一下午。', act:'肉丝套上去。丝袜磨龟头。撸。',     pool:['肉丝','尖头']},
  {t:'暖烘烘。刚脱下来的。',      act:'棉袜裹住整根。从根到头顶。来回。',      pool:['棉袜','短袜','罗纹袜']},
  {t:'皮筋味。袜口勒久了。',      act:'过膝袜套上去。袜口那一圈卡在冠状沟。撸。',    pool:['连裤袜','袜口','蕾丝链条','腿部']},
  {t:'没擦。汗干了又湿。',        act:'鼻子埋进腋窝。舌头伸出来。手撸快一点。',    pool:['蹲姿','未穿丝袜','凉鞋','凉拖','脚底']},
...
function action(pid){
  ...
  function tick(){
    ...
    gb.innerHTML='<div style="background:rgba(0,0,0,.45);border-radius:8px;padding:10px;text-align:center;"><div style="color:#ff6b9d;font-size:14px;">'+PAIR[pid].t+'</div><div style="color:#ffcc80;font-size:11px;margin:2px 0;">'+PAIR[pid].act+'</div>'+(mistakes>0?'<div style="color:#cc6666;font-size:10px;margin:1px 0;">配错'+mistakes+'次 +'+mistakes*10+'s</div>':'')+circ+'<div style="color:rgba(255,255,255,.35);font-size:11px;margin-top:3px;">'+MSG[mi%MSG.length]+'</div></div>';
